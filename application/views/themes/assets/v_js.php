...
if(isset($ASSETS)){
	...
	?>
		...
		<script type="text/javascript">
			...
			function file_all(form,id){
				...
				var extensions = /(\.jpg|\.jpeg|\.png|\.gif|\.doc|\.docx|\.xls|\.xlsx|\.ppt|\.pptx|\.pdf|\.txt|\.mp4|\.rar|\.zip)$/i;
	...
	if(in_array('html5-qrcode',$ASSETS)){
	?>
		<script type="text/javascript" src="{BASE_URL}themes/assets/html5-qrcode/html5-qrcode.min.js"></script>
		<script>
			...
			docReady(function () {
				...
				function onScanSuccess(decodedText, decodedResult) {
					//if (decodedText !== lastResult) {
						...
						$('#qr-reader-text').val(decodedText);

						//window.location=("<?php echo base_url(); ?>pages/content/scanner/action/scan/id/"+decodedText);
					//}
				}
	...
	if(in_array('html5-qrcode2',$ASSETS)){
		?>
			<script type="text/javascript" src="{BASE_URL}themes/assets/html5-qrcode/html5-qrcode.min.js"></script>
			<script>
				function docReady(fn) {
				...
				docReady(function () {
					...
					function onScanSuccess(decodedText, decodedResult) {
						//if (decodedText !== lastResult) {
							...
							// Handle on success condition with the decoded message.
							console.log(`Scan result ${decodedText}`, decodedResult);
							//$('#qr-reader-results2').html(decodedText);
	
							/* $.ajax({
								type: "POST",
								url: "{BASE_URL}api/html/scanner/code",
								data: {id: decodedResult},
								success: function(result){
									$("#qr-reader-results").html(result);
								}
							});
	
							console.log(`Scan result 2 ${decodedText}`, decodedResult); */

							var qrReaderText = $('#qr-reader-text').val();

							if(qrReaderText!=''){
								window.location=("<?php echo base_url(); ?>pages/content/scanner/action/scan/id/"+decodedText+"/resi/"+qrReaderText);
							}
