...
class M_sub_kriteria extends CI_Model {
	...
	public function start(){
		...
		$data['field_control'] = array(
			...
			'id_kriteria'=>array(
				'list'=>select("kriteria","*","WHERE flag_aktif='1' ORDER BY created"),
				'list_type'=>'2',
			),
		);
		$data['start'] = 1;
	...
	public function data(){
		...
		if($this->session->userdata('ei_login')!=FALSE){
			...
			$body = table(
				...
				nama,
				keterangan,
				(SELECT nama FROM kriteria WHERE $data[table].id_kriteria=kriteria.id_kriteria) as kriteria,
				flag_aktif",
				$data['condition'],
				$data['field_control'],
				$data['start'],
	...
	public function edit(){
		...
		if(!empty($action) && !empty($id)){
			...
			$data['field_control']['img_'.$data['table']]['attributes'] = '';
