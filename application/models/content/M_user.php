	...
	public function action(){
		...
		if(!empty($action)){
			...
			if($act=='add' || $act=='edit'){
				...
				# Jika add
				if($act=='add'){
					$input['masa_aktif'] = date('Y-m-d H:i:s',strtotime('+1 year'));
					...
					if($this->session->userdata('ei_login')==FALSE){
						...
						$id_new2 = generate_id("pasien");
						$created = date('Y-m-d H:i:s');
						$input['id_user_table'] = $id_new2;
						insert("pasien","","('$id_new2','$input[fullname]','$input[address]','$input[phone]','1','$created','$created')");

						/* $extension = array('jpg','jpeg','png','gif','doc','docx','xls','xlsx','ppt','pptx','pdf','txt','mp4');
						
						$file = basename($_FILES['img_umkm']['name']);
						$explode = explode('.', $file);
						$ext = end($explode);
						
						$img_umkm = date('YmdHis').'_'.str_rand(5).'.'.strtolower($ext);
						$tmpname = $_FILES['img_umkm']['tmp_name'];
						$path = 'upload/umkm/img_umkm/'.$img_umkm;
						if(in_array(strtolower($ext), $extension)){
							$upload = move_uploaded_file($tmpname, $path);
						}

						$file = basename($_FILES['file_situ']['name']);
						$explode = explode('.', $file);
						$ext = end($explode);

						$file_situ = date('YmdHis').'_'.str_rand(5).'.'.strtolower($ext);
						$tmpname = $_FILES['file_situ']['tmp_name'];
						$path = 'upload/umkm/file_situ/'.$file_situ;
						if(in_array(strtolower($ext), $extension)){
							$upload = move_uploaded_file($tmpname, $path);
						}
						
						insert("user","","('$id_new2','$input[fullname]','','','','1','$created','$created')"); */
					}
				}
				elseif($act=='edit'){
					$id = string(urinext('id'));
			...
			elseif($act=='activate'){
				if($this->session->userdata('ei_login')==FALSE){
					...
					$r = select2($data['table'],"*","WHERE username='$username' AND email='$email'");

					if($r!=NULL){
						update($data['table'],"status='3'","WHERE username='$username' AND email='$email'");
						$message[0] = 'Activate Success!';
						$type = 1;
						echo '
						<script>
							window.alert("Mohon Tunggu Konfirmasi Administrator !!!");
							window.location=("'.base_url().'pages/login");
						</script>';
						exit();
					}
						...
						<script>
							window.alert("Failed !!!");
							window.location=("'.base_url().'pages/activate");
						</script>';
						exit();
					}
				}
